...
name2iddis.each { tok, ids ->
  ...
  def flag = true
  ids.each { 
    if (id2sub[it]?.size()>1) { // not a leaf node
      flag = false
    }
  }
  if (flag) {
...
def fout = new PrintWriter(new BufferedWriter(new FileWriter("results/medline-simple")))
...
fout = new PrintWriter(new BufferedWriter(new FileWriter("results/medline-processed")))
